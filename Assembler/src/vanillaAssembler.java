...
public class vanillaAssembler {
    ...
    /**
     * main function which makes a new assmbler and calls its constructor with
     * the input arguments from terminal.
     */
    ...
    /**
     * constructor which gets the terminal inputs and make a readbuffer from
     * input and a output buffer for the data memory file which is shared among
     * kernels.
     */
    vanillaAssembler(String[] args) throws IOException {
        ...
        out_data = new BufferedWriter(new FileWriter("..\\Core\\" + args[1] + "_d.hex"));
    ...
    void processData(String sourceCode) {
        ...
        } /* Process the .fill keyword */ else if (sourceCode.toLowerCase().startsWith(".fillword")) {
            StringTokenizer st = new StringTokenizer(sourceCode, " ,\t");
            if (st.countTokens() != 3) {
    ...
    public void AssembleCode(String[] arg) throws IOException {
        ...
        while (sourceCodeLine != null) {
            if (isKeyword(sourceCodeLine)) {
                ...
                } /* Change the current code section to text */ else if (keyword.equalsIgnoreCase(".text")) {
                    currentCodeSection = 0;
                } /* Change the current code section to data */ else if (keyword.equalsIgnoreCase(".data")) {
